...
CREATE TABLE recipe (`id` int(10) NOT NULL AUTO_INCREMENT, `name` varchar(100) NOT NULL, `description` varchar(300), `category` varchar(50), `created_by_user_id` int(10) NOT NULL, `image_id` int(10), PRIMARY KEY (`id`), FOREIGN KEY (`created_by_user_id`) REFERENCES `users` (`id`) ON UPDATE CASCADE, FOREIGN KEY (`image_id`) REFERENCES `image` (`id`));
...
INSERT INTO instruction (id, recipeRank, description, image_id, recipe_id) VALUES (1, 1, 'First, place the roast in the roaster and fill with water until the water covers about half the pork. Cook on LOW for about 10 hours.', null, 1);
INSERT INTO instruction (id, recipeRank, description, image_id, recipe_id) VALUES (2, 2, 'Then, remove the roast form the heat and shred it, discarding as much fat as you can. You can then place the meat back into the roaster.', null, 1);